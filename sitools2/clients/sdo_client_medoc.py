...
def media_get_selection(server=None, media_data_list=[], download_type="TAR", **kwds) :
    ...
    media_data_list : list of Sdo_data objects
        ...
        Ex :
            sdo_data_list = media_search(DATES=[d1,d2], SERIES='hmi.m_720s', nb_res_max=10)
            media_get_selection(MEDIA_DATA_LIST=sdo_data_list,download_type='TAR')
    download_type : text
        Can be 'TAR' or 'ZIP' 
        Ex :
            media_get (MEDIA_DATA_LIST=sdo_data_list,DOWNLOAD_TYPE="tar")
...
#server
    ...
    if server is None :
        ...
        sys.stdout.write("server parameter not specified, default value is set : server=%s\n" % server)
    if server is not None and server not in allowed_server :
        raise ValueError("Server %s is not allowed for media_get_selection()\nServers available : %s\n" % (server,allowed_server))
...
######################CONTROL_START
    ...
    allowed_server=['http://medoc-sdo.ias.u-psud.fr',
...
def media_metadata_search(server=None, media_data_list=[],keywords=[], recnum_list=[],series=None, **kwds):
...
#Define dataset target 
    ...
    elif server.startswith('http://idoc-solar-portal') and series=='aia.lev1' :
...
##server
    ...
    if server=='http://medoc-sdo-test.ias.u-psud.fr' :
        ...
        metadata_ds=Sdo_IAS_SDO_dataset(server+"/webs_"+series+"_dataset")
