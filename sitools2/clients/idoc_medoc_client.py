...
def idoc_medoc_get(media_data_list=None, target_dir=None, download_type=None, **kwds):
    ...
    Example
    -------
    >>> sdo_data_list = idoc_medoc_search(
                            dates=[d1,d2],
                            series='hmi.m_720s',
                            nb_res_max=10)
    >>> idoc_medoc_get(MEDIA_DATA_LIST=sdo_data_list)

    >>> idoc_medoc_get(MEDIA_DATA_LIST=sdo_data_list, target_dir='results')

    >>> idoc_medoc_get (media_data_list=sdo_data_list,DOWNLOAD_TYPE="tar")
...
def idoc_medoc_get_selection(server=sitools2_url, media_data_list=None, download_type="TAR", **kwds):
    ...
    try:
        sdo_dataset = SdoIasSdoDataset(server + "/" + idoc_medoc_sdo_aia_dataset)
    except HTTPError:
        raise HTTPError
    else:
        if len(media_data_list) == 0:
...
def idoc_medoc_search(server=sitools2_url, dates=None, waves=None, series=None, cadence=None, nb_res_max=-1,
                      keywords=None, **kwds):
    ...
    keywords :  list
        ...
        Pay attention, in this case a dictionary is return with key, value element
    ...
    -------
    SdoData object list
    Dictionary object list if parameter keywords is used
    ...
    # Server definition
    # Define dataset url
    if server.startswith('http://idoc-medoc') and series.startswith('hmi'):
...
def idoc_medoc_metadata_search(server=sitools2_url, media_data_list=None, keywords=None, recnum_list=None,
                               series=None, **kwds):
    ...
    --------
        >>>sdo_data_list = idoc_medoc_search(
                                ...
                                SERIES='hmi.m_720s',
                                nb_res_max=10)
        >>>>meta = idoc_medoc_metadata_search(
                MEDIA_DATA_LIST=sdo_data_list,
...
# Initialize
    ...
    metadata_ds = None
    if server.startswith('http://idoc-medoc') and series == 'aia.lev1':
...
def metadata_info(server=sitools2_url, series='aia.lev1'):
    ...
    metadata_ds = None
    # Define dataset url
    if server == 'http://idoc-medoc-test.ias.u-psud.fr':
...
#        filename_path = None
    ...
    def metadata_search(self, server=sitools2_url, keywords=None, **kwds):
        ...
        if server_url.startswith('http://idoc-medoc'):
...
def main():
    ...
    recnum_list = []
    for sdo_data_item in sdo_data_list:
        try:
            recnum_list.append(sdo_data_item.recnum)
        except AttributeError:
            recnum_list.append(sdo_data_item['recnum'])
    print("recnum list : %s" % recnum_list)
    meta = idoc_medoc_metadata_search(keywords=['recnum', 'sunum', 'date__obs', 'quality', 'cdelt1', 'cdelt2',
                                                'crval1'], series="aia.lev1", recnum_list=recnum_list)
    for result in meta:
        print(result)
