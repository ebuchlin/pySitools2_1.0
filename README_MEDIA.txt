...
metadata information. You can also call the function get() to download the data you are interested in.
...
3-Use the function search() 
  ...
  sdo_data_list = search(DATES=[d1, d2],WAVES=[waves_value], CADENCE=[cadence_value], NB_RES_MAX=limit_value ) where :
  ...
  * Lower case as arguments are allowed too so 
    sdo_data_list=search(dates=[d1,d2],waves=[waves_value],cadence=[cadence_value], nb_res_max=limit_value) will do the same


Example: 
sdo_data_list=search(DATES=[d1,d2],WAVES=['335'],CADENCE=['2 min'], NB_RES_MAX=10) 
...
Since you have a result sdo_data_list from a previous search() request, you simply have to call the function get:

get (MEDIA_DATA_LIST=sdo_data_list,DECOMPRESS=decompress_value,TARGET_DIR=directory_name, QUIET=quiet_value)
...
sdo_data_list=search(DATES=[d1,d2],WAVES=['335'],CADENCE=['2 h']) 
